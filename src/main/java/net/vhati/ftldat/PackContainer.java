 ...
 * subdirectory "resources/" relative to FTLGame.exe. data.dat:data/
 * resource.dat:audio/ resource.dat:fonts/ resource.dat:img/
 *
 * FTL 1.6.1 switched to a single "ftl.dat", alongside "FTLGame.exe".
 * ftl.dat:audio/ ftl.dat:data/ ftl.dat:fonts/ ftl.dat:img/
 *
...
public class PackContainer {
	 ...
	 *
	 * Group1: parentPath/ (may be null). Group2: root/ (may be null). Group3:
	 * fileName (never null, if innerPath is valid).
	 */
	...
	/**
	 * Sets a pack to use for innerPaths within a given root dir, instead of the
	 * default pack.
	 *
	 * The root must include a trailing forward slash.
	 *
	 * A null pack should be interpreted as the result when paths should be ignored.
	 * A null root would apply to top-level files.
	 */
	...
	public List<AbstractPack> getPacks() {
		...
		for (AbstractPack pack : rootMap.values()) {
			if (pack != null && !result.contains(pack))
				result.add(pack);
		}
		if (defaultPack != null)
			result.add(defaultPack);
