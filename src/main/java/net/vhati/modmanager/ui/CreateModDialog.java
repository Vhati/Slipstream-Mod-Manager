...
public class CreateModDialog extends JDialog implements ActionListener {
	...
	public CreateModDialog(Frame owner, File modsDir) {
		...
		editorPanel.addTextRow(
				"Summary of gameplay effects; flavor; features; concerns about compatibility, recommended patch order, requirements; replaced ship slot; etc.");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == applyBtn) {
			...
			if (dirName.length() == 0) {
				JOptionPane.showMessageDialog(CreateModDialog.this, "No directory name was given.", "Nothing to do",
			...
			if (!genDir.exists()) {
				try {
					...
					JOptionPane.showMessageDialog(CreateModDialog.this,
							String.format("Failed to generate new mod: %s\n%s", genDir.getName(), f.getMessage()), "Error",
							JOptionPane.ERROR_MESSAGE);
				}
			} else {
				JOptionPane.showMessageDialog(CreateModDialog.this,
						String.format("A directory named \"%s\" already exists.", genDir.getName()), "Nothing to do",
						JOptionPane.WARNING_MESSAGE);
			}
		}
	}
