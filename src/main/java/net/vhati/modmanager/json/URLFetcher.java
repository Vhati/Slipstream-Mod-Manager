...
public class URLFetcher {
	 ...
	 *
	 * If the content has not changed, the destination file's modified date will be
	 * reset to the present time.
	 *
	 * If the content has changed, it will be written to the file, as will the new
	 * ETag.
	 *
	...
	public static boolean refetchURL(String url, File localFile, File eTagFile) {
		...
		try {
			...
			if (status >= 200 && status < 300) {
				...
				log.debug(
						String.format("No need to download \"%s\", the server's copy has not been modified", localFile.getName()));
