...
public class FTLUtilities {
	 ...
	 *
	 * Windows: Steam, GOG, HumbleBundle Linux (Wine): GOG, HumbleBundle Linux:
	 * Steam, HumbleBundle OSX: Steam, HumbleBundle
	 */
	...
	public static File promptForDatsDir(Component parentComponent) {
		...
		String message = "" + "You will now be prompted to locate FTL manually.\n"
		...
		fc.addChoosableFileFilter(new FileFilter() {
			...
			public boolean accept(File f) {
				return f.isDirectory() || f.getName().equals("ftl.dat") || f.getName().equals("data.dat")
	 ...
	 *
	 * FTL 1.01-1.5.13: Windows {FTL dir}/resources/*.dat {FTL dir}/FTLGame.exe
	 * Linux {FTL dir}/data/resources/*.dat {FTL dir}/data/FTL OSX {FTL
	 * dir}/Contents/Resources/*.dat {FTL dir}
	 *
	 * FTL 1.6.1: Windows {FTL dir}/*.dat {FTL dir}/FTLGame.exe Linux {FTL
	 * dir}/data/*.dat {FTL dir}/data/FTL OSX {FTL dir}/Contents/Resources/*.dat
	 * {FTL dir}
	 *
	 * On Windows, FTLGame.exe is a binary. On Linux, FTL is a script. On OSX,
	 * FTL.app is the grandparent dir itself (a bundle).
	 */
	 ...
	 *
	 * On Windows, "Steam.exe". On Linux, "steam" is a script. (
	 * http://moritzmolch.com/815 ) On OSX, "Steam.app" is a bundle.
	 *
	 * The definitive Windows registry will not be checked. Key,Name,Type:
	 * "HKCU\\Software\\Valve\\Steam", "SteamExe", "REG_SZ".
	 *
	 ...
	 *
	 * Steam will start, if not already running, and a popup with progress bar will
	 * appear.
	 *
	 * For FTL, this method amounts to running: Steam.exe "steam://validate/212680"
	 *
	...
	public static Process verifySteamGameCache(File exeFile, String appId) throws IOException {
		if (appId == null || appId.length() == 0)
			throw new IllegalArgumentException("No Steam APP_ID was provided");
	 ...
	 *
	 * On Windows, *.exe. On Linux, a binary or script. On OSX, an *.app bundle dir.
	 *
	 ...
	 *
	 * This view will not be jailed in Wow6432Node, even if Java is? Characters
	 * outside windows-1252 are unsupported (results will be mangled).
	 *
	 * Bad unicode example: "HKLM\\SOFTWARE\\Microsoft\\Windows
	 * NT\\CurrentVersion\\Console\\TrueTypeFont", "932", "REG_SZ".
	 *
	 * @param key       a backslash path starting with HKLM, HKCU, HKCR, HKU, HKCC
	 * @param valueName a value name, or "" for the "(Default)" value
	 * @param valueType REG_SZ ("Abc"), REG_DWORD ("0x1"), REG_BINARY ("44E09C"),
	 *                  etc
	 */
	public static String queryRegistryKey(String key, String valueName, String valueType) throws IOException {
		...
		try {
			...
			if (winDir != null && winDir.length() > 0) {
				// When Java's in Wow64 redirection jail, sysnative is a virtual dir with the
				// 64bit commands.
				// I don't know if this will ever happen to Java.
				File unWowRegExeFile = new File(winDir, "sysnative\\reg.exe");
				if (unWowRegExeFile.exists())
					regExePath = unWowRegExeFile.getAbsolutePath();
			}

			String[] steamRegArgs = new String[] { regExePath, "query", key, "/v", valueName, "/t", valueType };
			Pattern regPtn = Pattern.compile(Pattern.quote(((valueName != null) ? valueName : "(Default)")) + "\\s+"
					+ Pattern.quote(valueType) + "\\s+(.*)");
