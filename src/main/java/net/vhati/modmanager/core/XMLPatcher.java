...
public class XMLPatcher {
			...
			if (modFilter.matches(content)) {
				...
				if (!handled) {
					throw new IllegalArgumentException(
							String.format("Unrecognized mod tag <%s> (%s).", node.getName(), getPathToRoot(node)));
				}
			} else {
		...
		if (node.getNamespace().equals(modNS)) {
			...
			if (node.getName().equals("findName")) {
				...
				if (searchName == null || searchName.length() == 0)
					throw new IllegalArgumentException(
							String.format("<%s> requires a name attribute (%s).", node.getName(), getPathToRoot(node)));
				if (searchType != null && searchType.length() == 0)
					throw new IllegalArgumentException(String.format("<%s> type attribute, when present, can't be empty (%s).",
							node.getName(), getPathToRoot(node)));
				if (searchStart < 0)
					throw new IllegalArgumentException(
							String.format("<%s> 'start' attribute is not >= 0 (%s).", node.getName(), getPathToRoot(node)));
				if (searchLimit < -1)
					throw new IllegalArgumentException(
							String.format("<%s> 'limit' attribute is not >= -1 (%s).", node.getName(), getPathToRoot(node)));
				...
				if (panic && matchedNodes.isEmpty())
					throw new NoSuchElementException(String.format("<%s> was set to require results but found none (%s).",
							node.getName(), getPathToRoot(node)));
				...
				if (selectorNode != null) {
					for (Attribute attr : selectorNode.getAttributes()) {
						if (attr.getNamespace().equals(Namespace.NO_NAMESPACE)) {
							// Blank element values can't be detected as different from absent values (never
							// null).
							// Forbid "" attributes for consistency. :/
							if (attr.getValue().length() == 0)
								throw new IllegalArgumentException(String.format("<%s> attributes, when present, can't be empty (%s).",
										selectorNode.getName(), getPathToRoot(selectorNode)));
				...
				if (searchChildType != null && searchChildType.length() == 0)
					throw new IllegalArgumentException(String.format(
							"<%s> child-type attribute, when present, can't be empty (%s).", node.getName(), getPathToRoot(node)));
				if (searchStart < 0)
					throw new IllegalArgumentException(
							String.format("<%s> 'start' attribute is not >= 0 (%s).", node.getName(), getPathToRoot(node)));
				if (searchLimit < -1)
				...
				if (parNode == null)
					throw new IllegalArgumentException(
							String.format("<%s> requires a <par> tag (%s).", node.getName(), getPathToRoot(node)));
	...
	/**
	 * Returns collated find results (and par results, handled recursively), or null
	 * if node wasn't a par.
	 *
	 * Unique results from all finds will be combined and sorted in the order they
	 * appear under contextNode.
	 */
		...
		if (node.getNamespace().equals(modNS)) {
			...
			if (node.getName().equals("par")) {
				...
				if (parOp == null || (!parOp.equals("AND") && !parOp.equals("OR")))
					throw new IllegalArgumentException(
							String.format("Invalid \"op\" attribute (%s). Must be 'AND' or 'OR'.", getPathToRoot(node)));
				...
				for (Element criteriaNode : node.getChildren()) {
					...
					if (criteriaNode.getName().equals("par") && criteriaNode.getNamespace().equals(modNS)) {
						...
						if (candidates == null)
							throw new IllegalArgumentException(
									String.format("Invalid <par> search criteria <%s> (%s). Must be a <find...> or <par>.",
	 ...
	 *
	 * TODO: Maybe have handleModCommand() returning null when unrecognized, or an
	 * object with flags to continue or stop looping commands at contextNode (e.g.,
	 * halting after removeTag).
	 */
	protected void handleModCommands(Element contextNode, Element node) {
		...
		for (Element cmdNode : node.getChildren()) {
			...
			if (!handled) {
				throw new IllegalArgumentException(
						String.format("Unrecognized mod tag <%s> (%s).", cmdNode.getName(), getPathToRoot(cmdNode)));
			}
		}
	}
	...
	/**
	 * Returns the boolean value of an attribute, or a default when the attribute is
	 * null. Only 'true' and 'false' are accepted.
	 */
	protected boolean getAttributeBooleanValue(Element node, String attrName, boolean defaultValue) {
		...
		if (tmp.equals("true")) {
			...
			throw new IllegalArgumentException(String
					.format("Invalid boolean attribute \"%s\" (%s). Must be 'true' or 'false'.", attrName, getPathToRoot(node)));
		}
	}

	/**
	 * Returns the int value of an attribute, or a default when the attribute is
	 * null.
	 */
	protected int getAttributeIntValue(Element node, String attrName, int defaultValue) {
		...
		try {
			...
			throw new IllegalArgumentException(
					String.format("Invalid int attribute \"%s\" (%s).", attrName, getPathToRoot(node)));
		}
	}

	/**
	 * Matches elements with equal type/attributes/value. Null args are ignored. A
	 * blank type or value arg is ignored. All given attributes must be present on a
	 * candidate to match. Attribute values in the map must not be null.
	 */
	protected static class LikeFilter extends AbstractFilter<Element> {
		...
		public Element filter(Object content) {
			if (content instanceof Element == false)
				return null;
			Element node = (Element) content;
			String tmp;
	...
	/**
	 * Matches elements with child elements that match a filter. If the filter is
	 * null, matches all elements with children.
	 */
