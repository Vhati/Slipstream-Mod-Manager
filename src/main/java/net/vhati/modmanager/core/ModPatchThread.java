...
public class ModPatchThread extends Thread {
	...
	public ModPatchThread(List<File> modFiles, File datsDir, File backupDir, boolean globalPanic,
			ModPatchObserver observer) {
	...
	private boolean patch() throws IOException, JDOMException {
		...
		try {
			...
			for (BackedUpDat bud : backedUpDats) {
				if (bud.datFile.exists())
					bud.datFile.setWritable(true);
			}
			...
			if (ftlDatFile.exists()) { // FTL 1.6.1.
				...
				throw new IOException(String.format("Could not find either \"%s\" or both \"%s\" and \"%s\"",
						...
						if (fileName.endsWith(".xml.append") || fileName.endsWith(".append.xml")) {
							...
							if (!pack.contains(innerPath)) {
								...
								try {
									...
									InputStream mergedStream = ModUtilities.patchXMLFile(mainStream, zis, ultimateEncoding, globalPanic,
											pack.getName() + ":" + innerPath, modFile.getName() + ":" + parentPath + fileName);
									mainStream.close();
							...
							if (!pack.contains(innerPath)) {
								...
								try {
									mainStream = pack.getInputStream(innerPath);
									InputStream mergedStream = ModUtilities.appendXMLFile(mainStream, zis, ultimateEncoding,
											pack.getName() + ":" + innerPath, modFile.getName() + ":" + parentPath + fileName);
									mainStream.close();
									pack.remove(innerPath);
									pack.add(innerPath, mergedStream);
							...
							InputStream fixedStream = ModUtilities.encodeText(fixedText, ultimateEncoding,
									modFile.getName() + ":" + parentPath + fileName + " (with new EOL)");
							...
							InputStream fixedStream = ModUtilities.rebuildXMLFile(zis, ultimateEncoding,
	 ...
	 *
	 * If there is no collision, the innerPath is added to the known lists. A
	 * warning will be logged if a path with differing case exists.
	 *
	...
	private String checkCase(String innerPath, List<String> knownPaths, List<String> knownPathsLower) {
		if (knownPaths.contains(innerPath))
			return innerPath;
