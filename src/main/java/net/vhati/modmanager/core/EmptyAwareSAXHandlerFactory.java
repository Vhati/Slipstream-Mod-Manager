...
/**
 * A factory that produces SAXHandlers which distinguish tag-pairs from dual
 * tags.
 *
 * While this factory is registered with a SAXBuilder, strict parsing will occur
 * as normal, notifying the SAXHandler of progress. When the handler hears that
 * a tag ended at a different line/column than where it started, and no other
 * text was reported along the way, a zero-length text node will be added as
 * token content.
 *
 * Later, when the Element tree is printed, a special XMLOutputProcessor can
 * reproduce the tag pair pased on the presence of child Content objects - like
 * the dummy string - rather than the presence of non-whitespace text after
 * formatting.
 *
 * Note: Not all XMLReaders offer a Locator for line/col info. When that info is
 * not available, this handler will do nothing special.
 *
 * @see org.jdom2.input.SAXBuilder#setSAXHandlerFactory(SAXHandlerFactory
 *      factory)
 */
