...
public class FTLModManager {
	...
	public static void main(String[] args) {
		...
		// Doing this here instead of in "logback.xml", allows for conditional log
		// files.
	...
	private static void guiInit() {
		try {
			...
			if (appConfig.getProperty(SlipstreamConfig.STEAM_DISTRO, "").length() == 0) {
				int steamBasedResponse = JOptionPane.showConfirmDialog(null, "Was FTL installed via Steam?", "Confirm",
						JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
			...
			if ("true".equals(appConfig.getProperty(SlipstreamConfig.STEAM_DISTRO, "false"))) {
				...
				if (appConfig.getProperty(SlipstreamConfig.STEAM_EXE_PATH, "").length() == 0) {
					...
					if (steamExeFile == null && System.getProperty("os.name").startsWith("Windows")) {
						try {
							String registryExePath = FTLUtilities.queryRegistryKey("HKCU\\Software\\Valve\\Steam", "SteamExe",
									"REG_SZ");
					...
					if (steamExeFile == null) {
						...
						String steamPrompt = "" + "You will be prompted to locate Steam's executable.\n" + "- Windows: Steam.exe\n"
								+ "- Linux: steam\n" + "- OSX: Steam.app\n" + "\n"
				...
				if (appConfig.getProperty(SlipstreamConfig.STEAM_EXE_PATH, "").length() > 0) {
					...
					if (appConfig.getProperty(SlipstreamConfig.RUN_STEAM_FTL, "").length() == 0) {
						...
						int launchResponse = JOptionPane.showOptionDialog(null,
								"Would you prefer to launch FTL directly, or via Steam?", "How to Launch?", JOptionPane.DEFAULT_OPTION,
								JOptionPane.PLAIN_MESSAGE, null, launchOptions, launchOptions[1]);
						if (launchResponse == 0) {
			...
			if (askAboutUpdates) {
				String updatePrompt = "" + "Would you like Slipstream to periodically check for updates?\n" + "\n"
						+ "You can change this later.";

				int response = JOptionPane.showConfirmDialog(null, updatePrompt, "Updates", JOptionPane.YES_NO_OPTION,
			...
			System.gc();
			// System.exit( 1 ); // Don't do this (InterruptedException). Let EDT end
			// gracefully.
			return;
		}
	}
